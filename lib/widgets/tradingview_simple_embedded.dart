...
class TradingViewSimpleEmbedded extends StatefulWidget {
  final String symbol;
  final double height;
  ...
  const TradingViewSimpleEmbedded({
    super.key,
  ...
  @override
  State<TradingViewSimpleEmbedded> createState() => _TradingViewSimpleEmbeddedState();
}

class _TradingViewSimpleEmbeddedState extends State<TradingViewSimpleEmbedded> {
      ...
      child: Stack(
        children: [
          ...
          Container(
            ...
            child: _buildTradingViewContent(),
          ),
  ...
  Widget _buildTradingViewContent() {
    return Container(
      decoration: BoxDecoration(
        color: widget.theme == 'dark' ? Color(0xFF0F0F0F) : Colors.white,
        borderRadius: BorderRadius.circular(8),
  ...
  void _openTradingViewChart() async {
    HapticFeedback.lightImpact();
    
    // Create TradingView URL with proper parameters
    final tradingViewUrl = _getTradingViewUrl();
    
    try {
      final Uri url = Uri.parse(tradingViewUrl);
  ...
  String _getTradingViewUrl() {
    // Build TradingView URL with all the parameters
    final baseUrl = 'https://www.tradingview.com/chart/';
    final params = {
  ...
  void _showTradingViewDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: Text('Open TradingView'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Text('Click the button below to open TradingView in a new tab:'),
            SizedBox(height: 16),
            Container(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              Clipboard.setData(ClipboardData(text: _getTradingViewUrl()));
